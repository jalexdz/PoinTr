...
def save_single_ablation_boxplot(df, metric, out_path, title, ylabel):
    """Single-ablation boxplot (original style, no hue)."""
    ...
    ax.set_title(title or f'{metric.upper()} distribution across viewpoints (per asset)', fontsize=14)
...
def save_single_denoising_boxplot(partial_raw_by_asset, completion_raw_by_asset, out_path):
    """Per-ablation denoising boxplot (Partial vs Completion error)."""
    records = []
    ...
    sns.boxplot(
        data=plot_df, x="Asset", y="Error (mm)", hue="Source",
...
def save_combined_denoising_boxplot(all_denoising, out_path, ablation_order=None):
    """
    Combined denoising boxplot across all ablations.
    all_denoising: list of dicts {name, partial_raw_by_asset, completion_raw_by_asset}
    Shows hue = ablation, x = asset, one panel for Partial and one for Completion.
    """
    records = []
    for entry in all_denoising:
        abl = entry['name']
        for asset in sorted(entry['partial_raw'].keys()):
            for d in entry['partial_raw'][asset]:
                records.append({'Asset': asset, 'Ablation': abl, 'Source': 'Partial',    'Error (mm)': float(d) * 2})
            for d in entry['completion_raw'][asset]:
                records.append({'Asset': asset, 'Ablation': abl, 'Source': 'Completion', 'Error (mm)': float(d) * 2})
...
def _make_camera_params_from_gt(gt_pcd, cam_offset_factor=(0.8, -1.0, 1.6),
    ...
    return {"params": params, "center": center, "radius": radius, "cam_pos": cam_pos, "cam_up": cam_up}
...
def render_triplet_from_pcds(partial_pcd_path, gt_pcd_path, out_path,
    ...
    if include_error:
        ...
        pred_error_stats = {"mean": float(dists.mean()), "max": float(np.max(dists)), "min": float(np.min(dists))}
...
def run_single_ablation(cfg_path, ckpt_path, test_txt_path, ablation_name, out_path):
    ...
    Saves per-ablation individual plots and graphics.
    Returns (df, partial_raw_by_asset, completion_raw_by_asset).
    """
    print(f"\n=== Ablation: {ablation_name} ===")
    ...
    for sample in file_list:
        ...
        per_sample[asset].append({
            ...
            'partial_to_completion_mean':   2 * denoising['completion_to_gt_mean'],
            'partial_to_completion_std':    2 * denoising['completion_to_gt_std'],
            'partial_to_completion_median': 2 * denoising['completion_to_gt_median'],
        })
    ...
    save_single_denoising_boxplot(partial_raw_by_asset, completion_raw_by_asset,
    ...
    return df, partial_raw_by_asset, completion_raw_by_asset
...
def main(ablation_configs, out_path):
    ...
    for abl in ablation_configs:
        df, partial_raw, completion_raw = run_single_ablation(
    ...
    # ── Combined denoising boxplot ──
    save_combined_denoising_boxplot(
        all_denoising,
        os.path.join(combined_plot_dir, "denoising.pdf"),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "One ablation entry: NAME cfg_path ckpt_path test_txt_path. "
            "Repeat for each ablation, e.g.:\n"
            "  --ablation baseline cfgs/bl.yaml ckpts/bl.pth test.txt\n"
            "  --ablation ablation1 cfgs/a1.yaml ckpts/a1.pth test.txt"
        ),
    )
